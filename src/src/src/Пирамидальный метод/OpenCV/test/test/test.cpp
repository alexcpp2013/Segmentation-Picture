...
void help()
	{
	        printf("\nThis program demonstrated color pyramid segmentation cvcvPyrSegmentation() which is controlled\n"
	                        "by two trhesholds which can be manipulated by a trackbar. It can take an image file name or defaults to 'house0.jpg'\n"
	            "Usage :\n"
                        "./pyaramid_segmentation [image_path_filename -- Defaults to house0.jpg]\n\n"
                        );
	}
...
int main(int argc, char** argv)
{
  ...
  char* filename;
	    ...
	    l = 1;
	    threshold1 =255; //порог для установления связи
	    threshold2 =30;  //порог для кластеризации сегментов
